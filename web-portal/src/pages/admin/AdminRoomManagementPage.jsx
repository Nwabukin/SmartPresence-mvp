...
function AdminRoomManagementPage() {
  ...
  if (error && !rooms.length) {
    // Show error prominently if it occurs and no rooms are loaded
  ...
  return (
    <div>
      ...
      {rooms.length > 0 ? (
            ...
            {rooms.map((room) => (
              <tr key={room.room_id}>
                ...
                <td>
                  ...
                  <button onClick={() => handleDeleteClick(room.room_id)}>
                    Delete
                  </button>
                </td>
              </tr>
        ...
        !error &&
        !loading && <p>No rooms found. Click "Create New Room" to add one.</p>
      )}
    </div>
  );
}
