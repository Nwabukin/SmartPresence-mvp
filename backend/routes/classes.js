...
router.get('/', authMiddleware, async (req, res) => {
  if (![ROLES.ADMIN, ROLES.TEACHER].includes(req.user.role)) {
    return res.status(403).json({ error: 'Forbidden: Insufficient privileges to view classes.' });
  }
...
router.get('/:id', authMiddleware, validate(schemas.class.getById, 'params'), async (req, res) => {
  ...
  try {
    const result = await db.query('SELECT * FROM classes WHERE class_id = $1', [classId]);
...
router.put('/:id', authMiddleware, validate(schemas.class.getById, 'params'), validate(schemas.class.update), async (req, res) => {
  ...
  if (!name && !course_code && description === undefined) {
    return res.status(400).json({ error: 'No updateable fields provided (name, course_code, description).' });
  }
...
router.delete('/:id', authMiddleware, validate(schemas.class.getById, 'params'), async (req, res) => {
  ...
  try {
    ...
    if (result.rowCount === 0) {
       // Should not happen if previous check passed, but as a safeguard
      return res.status(404).json({ error: 'Class not found, could not delete.' });
    }
    // Cascade delete for enrollments and sessions is handled by DB constraints
    res.status(200).json({ message: `Class '${result.rows[0].name}' (ID: ${classId}) and all associated enrollments/sessions deleted successfully.` });
  } catch (err) {
...
router.post('/:classId/students', authMiddleware, async (req, res) => {
  ...
  if (isNaN(classIdInt) || isNaN(studentIdInt)) {
    return res.status(400).json({ error: 'Invalid class ID or student ID format.' });
  }
  ...
  try {
    ...
    const studentResult = await db.query('SELECT role FROM users WHERE user_id = $1', [studentIdInt]);
    if (studentResult.rows.length === 0) {
      return res.status(404).json({ error: 'Student not found.' });
    }
    if (studentResult.rows[0].role !== ROLES.STUDENT) {
      return res.status(400).json({ error: 'Invalid action: User is not a student.' });
    }
    ...
    if (err.code === '23505') { // Unique violation (student already enrolled)
      return res.status(409).json({ error: 'Conflict: Student is already enrolled in this class.' });
    }
    console.error(`Error enrolling student ${studentIdInt} in class ${classIdInt}:`, err);
...
router.delete('/:classId/students/:studentId', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (unenrollmentResult.rowCount === 0) {
      return res.status(404).json({ error: 'Enrollment record not found. Student may not be enrolled in this class.' });
    }
    res.status(200).json({ message: 'Student unenrolled successfully.', details: unenrollmentResult.rows[0] });
  } catch (err) {
    console.error(`Error unenrolling student ${studentIdInt} from class ${classIdInt}:`, err);
