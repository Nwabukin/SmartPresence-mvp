# SmartPresence MVP: Development Process

This document outlines the tasks involved in developing the SmartPresence MVP, an attendance management solution.
...
*   **Task 0.1: Version Control Setup**
    ...
    *   Define branching strategy (e.g., Gitflow).
        *   The project will use the Gitflow branching model.
        *   **`main`**: Production-ready code. Only release and hotfix branches are merged here.
        *   **`develop`**: Main development branch. All feature branches are merged here. This branch is created from `main` and `develop` is the base for feature branches.
        *   **`feature/<feature-name>`**: For new feature development. Branched from `develop` and merged back into `develop`.
        *   **`release/<version>`**: For preparing a new production release. Branched from `develop`, then merged into `main` and `develop`.
        *   **`hotfix/<fix-name>`**: For critical production bug fixes. Branched from `main` and merged back into both `main` and `develop`.
*   **Task 0.2: Project Structure and Tooling**
    *   Create main project directory and subdirectories for backend, web-portal, and mobile-app.
    *   Set up linters and formatters for each sub-project (e.g., ESLint, Prettier for Node.js/React; Dart/Flutter linters).
...
*   **Task 0.4: Initial Web Portal Project Setup (React)**
    *   Initialize React project (e.g., `create-react-app` or Vite).
...
*   **Task 7: Setup React Project & Basic Structure**
